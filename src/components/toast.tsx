...
export const useToast = create<ToastStore>((set) => ({
  ...
  addToast: (toast) => {
    ...
    setTimeout(() => {
      set((state) => {
        if (!state.toasts.some(({ id, timestamp }) => id === toast.id && timestamp === toast.timestamp)) {
          return state;
        }
        return {
          ...state,
          toasts: state.toasts.filter(({ id, timestamp }) => id !== toast.id || timestamp !== toast.timestamp),
        };
      });
    }, 2500);
  },
...
export const Toast: FC = () => {
  ...
  return (
    <>
      <div aria-live="assertive" className="pointer-events-none fixed inset-0 z-20 flex items-end px-4 py-6 sm:p-6">
          ...
          {[...new Array(5)].map((_, index) => {
            ...
            return (
              <Transition
                ...
                leaveTo="opacity-0"
              >
                <figure className="button-border pointer-events-auto flex w-full max-w-sm overflow-hidden rounded-lg border-gray-400/30 bg-white p-4 shadow-gray-500/10 shadow-xl">
                  {toast?.error ? (
                  ...
                  <p className="ml-3 w-0 flex-1 pt-0.5 font-medium text-gray-900 text-sm">{toast?.message}</p>
