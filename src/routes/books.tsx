        ...
        return (
          ...
          >
            ...
            <linearGradient id={`${gradientId}-${index}`}>
              ...
              <stop offset={`${percentage}%`} className={clsx("[stop-color:theme(colors.yellow.500)]")} />
              <stop offset={`${percentage}%`} className={clsx("[stop-color:theme(colors.gray.300)]")} />
  ...
  return (
    <figure className="group spacing-2">
      <div className="relative grid">
        ...
        <footer className="absolute right-4 -bottom-2 flex gap-2">
          {book.rating ? (
          ...
          <button
            type="button"
            className="flex items-center justify-center gap-1 rounded-md bg-white px-1.5 py-1 h:text-red-500 text-gray-300 drop-shadow-md transition-all"
            disabled={book.upvoted}
            onClick={() => addVote(book.id)}
          >
            <HeartIcon className="h-5 w-5" />
            <span className="font-semibold text-sm">{book.votes}</span>
          </button>
        </footer>
      </div>
      <figcaption className="">
        <h2>
          <Link href={book.url ?? ""} className="font-semibold d:text-gray-100 text-[15px] text-gray-900 hfa:underline">
            {book.name}
          </Link>
        </h2>
        <h3>
          {" "}
          <Link href={book.author_url ?? ""} className="d:text-gray-400 text-[15px] text-gray-900 tracking-tight hfa:underline">
            {book.author}
          </Link>
        </h3>
      </figcaption>
