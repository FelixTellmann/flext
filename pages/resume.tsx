...
const ResumeSection: FC<PropsWithChildren<{ title: string }>> = ({ title, children }) => {
  ...
  return (
    <section
      ...
      className="-mx-4 scroll-mt-[122px] overflow-hidden px-4 spacing-4 print:!overflow-visible md:overflow-visible"
      ref={sectionRef}
    >
...
const ResumeSectionDateSidebar: FC<ResumeSectionDateSidebarProps> = ({
  ...
  return (
    <aside className="absolute top-1 left-[4.5rem] hidden h-full print:!left-4 print:!block md:block xl:left-6 2xl:left-4 ">
      <h3 className="absolute top-0 right-full pr-6 text-right text-xs font-medium leading-[16px] text-gray-400 print:!pr-8 xl:pr-8">
        {showDateRange
          ? <span className="spacing-1">
              <span className="whitespace-nowrap print:!mr-0 xl:mr-2">
...
const ResumeFooter = () => {
  ...
  return (
    <footer
      className={clsx(
        "fixed inset-x-0 bottom-0 z-40 px-4 py-2 transition-all ease-linear [--resume-footer-bg:theme(colors.gray.50)] print:!hidden md:hidden",
...
export const Resume: FC = (props) => {
  ...
  return (
    <>
      ...
      <tbody className="block print:table-row-group print:!break-after-page">
        <tr className="block print:!table-row">
          <td className="block print:!table-cell">
            <article className="relative mx-auto mb-16 grid max-w-6xl gap-12 px-4 py-16 print:!flex  print:!py-4 print:!pl-24 md:px-8 lg:grid-cols-[1fr_200px] print:[&_*]:![-webkit-print-color-adjust:exact] print:[&_*]:![color-adjust:exact] print:[&_*]:![print-color-adjust:exact]">
              ...
              <aside className="sticky top-[144px] mb-auto hidden max-h-min print:!hidden lg:spacing-8">
