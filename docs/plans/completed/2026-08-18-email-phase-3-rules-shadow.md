...
**Post-ship: final whole-branch review, 2026-08-19**

The plan was moved to `completed/` at `052d511`; a final whole-branch review then ran across this phase's 25 files and returned FIX FIRST. All safety invariants passed — but three findings landed, and one was the pattern the review had been asked to hunt for. Fixed in one wave (`cc5c95d`, `80c0e1f`, `c45f79d`, `e02559c`), re-reviewed once, and cleared as SAFE TO MERGE.

- **A fifth spelling divergence, and this one genuinely diverged.** §5.1 defines `is_automated` as an `Auto-Submitted` header **or** a `no-reply@` / `mailer-daemon@` / `postmaster@` / `do-not-reply@` local part. `signals.ts` implemented both halves; the SQL in `needs-action.ts` tested only the header. Measured against production: **1,156 of 5,000 queue rows — 23% — came from senders the rules engine classifies as automated**, so the two surfaces the operator reviews side by side disagreed about a quarter of the queue. The comment above that code asserted no automated input could reach it, which was false and is what let it hide. Fixing it revealed a **third** spelling in `senders.ts`'s `automated_count`, where `no-reply@listifyregistry.com` had been reporting 0 of 523 messages as automated. One definition now lives in `signals.ts` and is bound as a parameter into the SQL rather than retyped. **Queue: 7,536 → 5,616 unfiltered (−25.5%), 617 → 401 at the 30-day default (−35.0%).**
- **The Sender surface reported "suppressed" where the engine would act.** `resolveSuppression` returned `derived_allowlist` for any `auto_trash` policy with `my_reply_count > 0`, ignoring scope — but §5.3 makes an explicit address-level policy outrank the scoped guards, so such a policy displayed as inert while the engine would trash the mail. Latent (`SenderPolicy` held 0 rows and the seed creates no `auto_trash`) but live on the first one added, on the most dangerous action class. Fixed by importing the engine's own `isBlocked` rather than paraphrasing it; the client bundle was verified free of `drizzle`/`mysql2` afterwards, since `guards.ts` and `signals.ts` have no runtime imports at all.
- **This document's own "Known gaps" was stale and false**, claiming `last_in_thread_is_mine` unimplemented when Tasks 5 and 6 had both built it — corrected above, since it would have sent Phase 4 rebuilding it.

Also fixed: an invented "for an open thread" qualifier in a `guards.ts` comment that the code did not implement, and two unchecked `as` casts over `varchar` columns replaced with real narrowings.

Verified after the fix wave: `sh tmp/tsc-mine.sh` clean in scope, `bunx biome check` clean across all touched files, **`bun test` 149 pass / 0 fail** (up from 136 — five new sender-guard cases and eight asserting the SQL and TypeScript halves of `is_automated` agree on anchoring, case and nulls), `bun run build` succeeding. The safety wall was re-verified independently and holds: no provider import in `server/mail/shadow/`, all five `getMailboxLock` calls read-only, no mailbox mutation anywhere in `server/mail`, every `rejectUnauthorized` true, nothing deleting from `message`, and `status` only ever `shadow` by construction.

Still open, unchanged: no browser has rendered any of the three surfaces; no shadow pass has run; migration 0004 and the seed are both unapplied.
