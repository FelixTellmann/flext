...
`tsconfig.json` has `include: ["**/*.ts", …]` and `types: ["node", "vite/client", "vite-plugin-svgr/client"]`, so `bun run tsc` typechecks test files but cannot resolve `bun:test`. Without this step, Step 5 fails with `Cannot find module 'bun:test'`.
