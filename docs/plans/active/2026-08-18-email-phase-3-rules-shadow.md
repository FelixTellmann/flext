...
- **`last_in_thread_is_mine` is still not implemented.** Phase 2's review established the Sent-folder exclusion does not approximate it. `thread_state` gives the machinery; this plan does not assign it, and it must not be claimed as done.
